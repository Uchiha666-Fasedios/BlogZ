...
<div class="container">
        ...
        <div class="col-md-10">
			...
			<div class="card pub_image pub_image_detail">
				...
				<div class="card-body">
					...
					<iframe width="600" height="338" src="{{asset('../storage/app/images/'. $video->video_path )}}" frameborder="0" allowfullscreen></iframe>
          ...
          <?php ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// ?>
            ...
            @if($user_like)
          <img src="{{asset('img/heart-red.png')}}" data-id="{{$video->id}}" class="btn-like" ><?php //asset en laravel es ir al directorio public ?>
          @else
          <img src="{{asset('img/heart-black.png')}}" data-id="{{$video->id}}" class="btn-dislike" ><?php //asset en laravel es ir al directorio public ?>
          @endif
          <span class="number_likes">  {{count($video->likes)}}<?php //cuenta la cantidad de likes de tal imagen gracias a la ORM?></span>
          </div>

					@if(Auth::user() && Auth::user()->id == $video->user->id)
					<div class="actions">


						<a href="{{route('video.edit',['id'=>$video->id])}}" class="btn btn-sm btn-primary">Actualizar</a>
  ...
  <?php //ESTE CODIGO ESTA HECHO POR BOOSTRAP 4 SE LLAMA MODAL?>
							...
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-footer">
										...
										<a href="{{route('video.delete',['id'=>$video->id])}}" class="btn btn-danger">Borrar definitivamente</a>
									</div>
					...
					<div class="comments">
						...
						<h2>Comentarios ({{count($video->comments)}})</h2>
						<hr>

						<form method="POST" action="{{ route('commentVideo.save') }}">
							@csrf
...
<?php //muestro lista de los comentarios ?>
          ...
          <?php   //este boton aparecera cuando Auth::check() me saca si estoy identificado y el logeado sea el duenio de la imagen o del comentario?>
								@if(Auth::check() && ($comment->user_id == Auth::user()->id || $comment->video->user_id == Auth::user()->id))
