...
if (is_object($this->getUser())) {//getUser si este metodo q esta en el controlador base nos devuelve un objeto (significa q el usuario esta logeado)
    return $this->redirect('home');
        ...
        $error = $authenticationUtils->getLastAuthenticationError();//getLastAuthenticationError me saca el error 
		
		$lastUsername = $authenticationUtils->getLastUsername();


        return $this->render('AppBundle:User:login.html.twig', Array( //me voy a esta vista llevandome la variable para mostrarla ahi
            'error' => $error,
...
if ($status) {
    $this->session->getFlashBag()->add('status', $status);// add('message', 'Animal creado') le estoy agregando el mensaje 
        ...
        return $this->render('AppBundle:User:register.html.twig', Array( //me voy a esta vista llevandome la variable para mostrarla ahi
...
$user->setImage($file_name);
  ...
  $user->setImage($file_user);
    ...
    if ($status) {
        ...
        return $this->render('AppBundle:User:edit_user.html.twig', Array( //me voy a esta vista llevandome la variable para mostrarla ahi
            'form' => $form->createView()//pasandole el formulario .. createView me genera el html para imprimir el formulario
        )); 
    }
    ...
    if ($form->isSubmitted()) {
        if ($form->isValid()) {
            ...
            if (($user->getEmail() == $user_isset[0]->getEmail() && $user->getNick() == $user_isset[0]->getNick()) || count($user_isset) == 0) {
                ...
                $flush = $em->flush();
                if ($flush == null) {
                    $status = "Sus datos han sido modificados correctamente";
                } else {
                    $status = "Sus datos no han sido actualizados";
                }
            } else {
                $status = "Nick o Email ya existente";
            }
        } else {
            $status = "Sus datos no han sido actualizados";
        }
...
$dql= "SELECT p FROM BackendBundle:Publication p WHERE p.user = $user_id ORDER BY p.id DESC"; //para q me tome $user_id ay q poner comillas dobles ""
    $query= $em->createQuery($dql);
   $paginator=$this->get('knp_paginator');//$this->get('knp_paginator') llamo al servicio de paginacion
   $pagination=$paginator->paginate(
    $query,$request->query->getInt('page',1),5  //va a cargar el parametro page y si no tiene nada page carga el 1.. 5 son los registros q se van a mostrar por pagina
