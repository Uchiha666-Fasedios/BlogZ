...
Route::middleware(['auth','role:administrador'])->group(function(){//role es el midelware y administrador es el parametro q quiero q tenga..hago un so mas profecional con el middleware y se usa el ROLE en la explicacion de laravel se explica.. Resumiendo lo q hago es q solo accederan a estas rutas los autenticados y q tengan el rol de administrador
...
// Slider
Route::resource('admin/slider','admin\SliderController')->only(['index','store','destroy']);//only con esto le aclaro las acciones q van a invocar a este controlador
